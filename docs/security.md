...
**Phase: 1 (network isolation), 2 (JWT scope check at Athena MCP).** Phase 1 relied on Proxmox firewall rules restricting which guests can reach Athena's inference port. Correction 2026-07-29: Athena as built enforces **bearer-token RBAC on every call** (tokens, roles, per-token surfaces) plus per-principal server-side budgets (Athena ADR 041) — the "network-trust only" premise understates the live daemon. Zakros-side JWT scoping at the Athena MCP remains the Phase 2 design. Phase 2 adds JWT validation at the Athena MCP, depending on §6 (MCP Broker Authentication) landing.
...
- Direct inference API vs MCP on Athena: Athena exposes two caller-authentication surfaces. The inference API (:7447, bearer-token RBAC) is reachable directly from local-model-backend pods (Iris §10, and the Phase 2 pod fleet — Themis §11, Momus §12, Calliope §13, Prometheus §14) gated by network trust plus Athena's own token check. The Athena MCP fronts the higher-privilege surfaces (`models.pull`, `models.load`, `sandbox.*`, `corpus.refresh`) and the JWT-scoped inference path (`inference.query`) that Pythia §9 uses for broker-fronted summarization. Two patterns coexist by design. Phase 2 work: confirm the tokens issued to Labyrinth pods carry inference-only surfaces (Athena's per-token surface scoping supports this) and that control-plane routes (`/api/*`) are not reachable with pod tokens.
