...
## Addendum 2026-07-28 (evening) — Centaur stress-tested against its repo; two corrections to §1

Operator challenged §1's competitor claim ("the README looks like a Slack frontend for agents, not an SDLC-in-a-box"). Stress test ran against the actual repo (`paradigmxyz/centaur`, read 2026-07-28) rather than our summary of it. The competitor claim **survives**, but §1 was wrong in both directions:

**Correction 1 — §1 understated Centaur's SDLC coverage.** The comparison table says chat dispatch is "Slack-native, thread-per-session." Stale: `services/githubbot` landed 2026-07-07 → 2026-07-20 — *after* the July 4 survey, missed by this delta — and it is a genuine SDLC loop, not chat ingress. Verified from its README: issue assigned to the bot → agent implements a fix in its sandbox and opens a PR, self-assigns, and manages it toward merge; review-requested → isolated review session posts inline comments + summary via `gh` (this is Momus/L2, shipped, while ours is not started); PR self-management fixes red CI (bounded retries, then tags a human), addresses `changes_requested` with one commit and re-requests review, and **auto-merges by default** when `mergeable_state == clean`. Webhook signatures verified, deliveries de-duplicated, turns gated by GitHub `author_association`. Teams and Linear ingress also exist. Centaur went from "no SDLC story" to issue→implement→PR→review→CI-fix→merge in two weeks.

**Correction 2 — §1 overstated Centaur's credential model.** "Stronger than Hecate's, not weaker" is only half true. Centaur's own `security.mdx`: *"Credentials are deployment-scoped, not yet user-scoped… a single vault that every sandbox in the deployment draws from, so a tool's reach is the same regardless of which user invokes it"* — per-user/per-channel scoping is roadmap. The strengths are orthogonal: Centaur has better **delivery** (sandbox only ever sees placeholders; iron-proxy substitutes on the wire), Zakros has better **scoping** (per-task JWT scopes, per-credential OpenBao policies, short TTLs). The H2b takeaway stands but sharpens: steal the substitution mechanism, keep our scoping — not "adopt their model."

**What the stress test confirmed is still Zakros-only:** typed task envelopes and pod classes (Centaur's review/issue methodologies are replaceable prompt strings — `GITHUBBOT_REVIEW_PROMPT`); the approval philosophy (Centaur auto-merges its own PRs by default; Zakros's invariant is human-gated merge + confirmation tokens for high-blast ops — a positioning difference, not a feature gap); episodic cross-run memory (Centaur persists per-thread transcripts, no semantic recall across tasks); Argus-style stall/runaway supervision; hypervisor blast-radius split; subscription-OAuth economics; in-boundary inference appliance.

**Narrative consequence (supersedes P2-7's framing):** the differentiator is no longer "they don't do SDLC" — it is "they do SDLC with prompt-string methodology, deployment-wide credentials, and auto-merge; Zakros does it with typed tasks, scoped credentials, and human gates."
